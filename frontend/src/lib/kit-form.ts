...
// One titled section of a kit show/edit page. Either names static fields (a
// name the metadata does not publish, or does not publish as editable, is
// dropped) or asks for the cube's runtime custom fields (rendered from the
// metadata alone, so a field defined after this code was written appears here).
export type FieldGroup =
  | { title: string; fields: string[] }
  | { title: string; custom: true }

export type GroupedFields = { title: string; fields: FieldMetadata[] }

export function groupedFieldsOf(
  groups: readonly FieldGroup[],
  meta: { fields: FieldMetadata[] },
): GroupedFields[] {
  const resolved: GroupedFields[] = []
  for (const group of groups) {
    const fields =
      "custom" in group
        ? meta.fields.filter((f) => f.custom && f.editable)
        : group.fields.flatMap((name) => {
            const field = meta.fields.find((f) => f.name === name && f.editable)
            return field ? [field] : []
          })
    if (fields.length > 0) resolved.push({ title: group.title, fields })
  }
  return resolved
}
